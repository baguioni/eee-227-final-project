...
def assemble(assembly_code):
    machine_code = []
    instruction_pair = [None, None]  # [load/store, arithmetic]
    ...
    for line_number, line in enumerate(assembly_code, start=1):
        ...
        try:
            ...
            instruction_type = line.split()[0].upper()

            if instruction_type in {'LOAD', 'STORE'}:
                if instruction_pair[0] is None:
                    instruction_pair[0] = binary_instruction
                else:
                    machine_code.append(instruction_pair[0] + '0000000000000000')
                    instruction_pair[0] = binary_instruction
            elif instruction_type in {'AND', 'OR', 'XOR', 'ADD', 'SUB', 'MUL'}:
                if instruction_pair[1] is None:
                    instruction_pair[1] = binary_instruction
                else:
                    machine_code.append('0000000000000000' + instruction_pair[1])
                    instruction_pair[1] = binary_instruction

            if instruction_pair[0] is not None and instruction_pair[1] is not None:
                machine_code.append(instruction_pair[0] + instruction_pair[1])
                instruction_pair = [None, None]

        except ValueError as e:
            raise ValueError(f"Error on line {line_number}: {e}")

    # Handle any remaining instructions in the pair
    if instruction_pair[0] is not None:
        machine_code.append(instruction_pair[0] + '0000000000000000')
    if instruction_pair[1] is not None:
        machine_code.append('0000000000000000' + instruction_pair[1])
